...
const loginUser = async (req, res) => {
  try {
    ...
    //Con el ID de empresa buscamos la moneda
    const currency = await prisma.company.findFirst({
      where: {
        id: user.companyId
      },
      select: {
    ...
    res.status(200).json({ userInfo: user, token: token, currency: currency }) // Por aca se pasará el token al front cuando se conecte
  } catch (error) {
    console.error('Error al registrar el usuario:', error)
    res.status(500).json({
      error: 'Hubo un error al registrar el usuario.'
...
const editUser = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json('Ocurrió un error al actualizar la informacion del usuario')
  }
}
...
const deleteUser = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json('Se ha producido un error al intentar borrar el usuario')
  }
}
